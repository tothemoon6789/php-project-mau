        ...
        <div class="row">
            <div class="col-12">
                ...
                @if (isset($row))
                    ...
                    <form action="{{ route('adminsanpham.san-pham.edit',["id"=>$row->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('GET')
                        <div class="form-group">
                            ...
                            <select class="form-control" name="danhmuccaphai" id="danhmuccaphai">
                                ...
                                @foreach ($danhmuccaphai as $i)
                                        <option value="{{$i->id}} " {{$i->danhmuccaphai === $row->danhmuccaphai ? 'selected' : ''}}>{{ $i->danhmuccaphai }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <input value="{{ $row->tieude }}" type="text" class="form-control" name="tieude"
                                id="tieude" aria-describedby="helpId" placeholder="Nhập sản phẩm">
                        </div>
                        <div class="form-group">
                            <label for="gia">Giá</label>
                            <input value="{{$row->gia}}" type="text" class="form-control" name="gia" id="gia" aria-describedby="helpId"
                        ...
                        <div class="list-images">
                            @if (isset($row) && !empty($row))
                                @foreach (json_decode($row->hinhanh) as $key => $img)
                                    <div class="box-image">
                                        <input type="hidden" name="images_uploaded[]" value="{{ $img }}"
                                            id="img-{{ $key }}">
                                        <img src="{{ asset('../../../../../uploads/'.$img) }}" class="picture-box">
                                        <div class="wrap-btn-delete"><span data-id="img-{{ $key }}"
                                                class="btn-delete-image">x</span></div>
                                    </div>
                                @endforeach
                                <input type="hidden" name="images_uploaded_origin" value="{{ $row->hinhanh }}">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="mota" id="mota" rows="10">{{ $row->mota }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="noidung">Nội dung</label>
                            <textarea class="form-control" name="noidung" id="noidung" rows="30">{{ $row->noidung }}</textarea>
                        </div>

                        <button type="submit" class="btn btn-primary btn-block mt-2 mb-5">Cập nhật</button>
                    </form>
                @endif
            </div>
        </div>
